...
This project provides a simple voice transcription tool that utilizes the `WhisperModel` for speech recognition. The application records audio when the scroll-lock is pressed, saves the audio to a temporary file, transcribes the audio using a Whisper model, and then saves the transcription to a text file. Additionally, the transcribed text is copied to the clipboard and can be automatically pasted into any application where the cursor is active.

Additionally, an audio file can be used as a parameter for the automatic transcription of long recordings.
